from clients.auth.auth_schema import RegisterResponseSchema, TokenSchema, GetMeResponseSchema
...
def assert_get_me_response(response: GetMeResponseSchema):
    """
    Проверяет корректность ответа при запросе авторизированного пользователя о себе.
